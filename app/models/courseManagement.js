...
coursemManagementSchema.pre("save", async function (next) {
  ...
  var doc = this;
  let counter = await InternalServices.getSequenceId({
    type: "courseManagement",
  });
  doc.courseManagementId = (counter?.data?.count + 1)
    .toString()
    .padStart(6, "0")
    .toString();
  next();
...
const courseManagement = mongoose.model(
  "courseManagement",
  coursemManagementSchema
);
